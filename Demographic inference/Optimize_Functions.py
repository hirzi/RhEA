...
def parse_params(param_number, in_params=None, in_upper=None, in_lower=None):
    ...
    #lower bounds
    if in_lower is None:
        lower_bound = [0.001] * param_number
...
def Optimize_Routine(fs, outfile, model_name, func, rounds, param_number, fs_folded=False, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    #--------------------------------------------------------------------------------------
    # Mandatory Arguments =
    ...
    #(7) fs_folded: A Boolean value (True or False) indicating whether the empirical fs is folded (True) or not (False). Default is False.
    ...
    for r in range(rounds):
        ...
        for rep in range(1, (reps_list[r]+1) ):
            ...
            #params_opt = moments.Inference.optimize_log_fmin(params_perturbed, fs, func, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "{}.log.txt".format(model_name))
            params_opt = moments.Inference.optimize_log(params_perturbed, fs, func, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "{}.log.txt".format(model_name))
