...
############## This first section calculates SNP environmental associations via gradient forest ##############

## Read arguments from command line
args <- commandArgs(trailingOnly = TRUE)
run_number <- as.numeric(as.character(args[1]))
...
spatialCovar <- "LATLON" # whether the spatial covariate is longitude-latitude (LATLON) or Moran's Eigenvector Maps (MEM)
# Stop script execution if run_number is invalid (for runs on Euler server)
if (run_number > ceiling(number_total_SNPs/num_SNPs_perRun)) {
  stop("run_number is invalid. Please define run_number < number_total_SNPs/num_SNPs_perRun")
}
...
pop_geodata <- read.csv(paste0(working_dir,"popInfo.csv"))
...
if (combined_gradientForest == FALSE) { 
  ...
  # library(adegraphics)
  # library(spdep)
  # # Here, we consider the overlap of paths (i.e. overlap of dispersal routes; van Etten & Hijmans 2010) departing from a common origin (the refugia), across a spatial transition matrix (resistance (cost)/conductance surface) defined by the lineage-specific SDM projection (1-predicted probability of presence). Two algorithms are available: random walk and randomised shortest paths (controlled by the theta paratmeter). 
  # # We also considered great circle, least-cost paths, resistance and overlap distances 
  # # These various distances matrices have been calculated via IBD_n_He_SDM_NatCommRev.R, and their results (distances matrices) are imported below.
  # #greatCircleDistances <- readRDS(paste0(working_dir,"greatCircle_distMat_43GFpops.rds"))
  # #leastCosts <- readRDS(paste0(working_dir,"leastCosts_distMat_43GFpops.rds"))
  # #resistanceDistances <- readRDS(paste0(working_dir,"resistance_distMat_43GFpops.rds"))
  # traj_overlap <- readRDS(paste0(working_dir,"trajOverlap_distMat_43GFpops.rds")) # Here, I assume traj_overlap[[1]] returns the overlap and traj_overlap[[2]] returns the non-overlap.
  
  # Import population coordinates.
  # Note populations were ordered differently in IBD_n_He_SDM_NatCommRev.R, constrained by the FST matrix, hence why we take the circuitious step to import another population table here and reorder later.
  #pop_geodata_allCentralAlps <- read.table("/Users/luqman/Documents/Hirzi/ETHPHD/PopulationGenetics_wholeGenome/IBD/Pops_CentralAlps_He_GL2.txt",header=T,sep='\t')
  pop_geodata_allCentralAlps <- read.table(paste0(working_dir,"Pops_CentralAlps_He_GL2.txt"),header=T,sep='\t')
  ## Subset to 43 gradient forest populations
  pops_toRemove <- setdiff(pop_geodata_allCentralAlps$Pop, pop_geodata$pop)
  remove_lists <- lapply(pops_toRemove, function(x) which(pop_geodata_allCentralAlps$Pop != x))
  remove_list<-seq(1:length(pop_geodata_allCentralAlps$Pop))
  for (a in remove_lists) {
    remove_list<-intersect(a,remove_list)
  }
  pop_geodata_alt <- pop_geodata_allCentralAlps[remove_list,]
  pop.coords <- pop_geodata_alt[,c("lon","lat")]
  pop.sp <- pop.coords
  ## Convert the dataframe to a spatial points dataframe, and add CRS projection
  prj_longlat <- "+init=epsg:4326"
  coordinates(pop.sp) <- ~lon+lat
  crs(pop.sp) <- prj_longlat
  
  if (spatialCovar == "MEM") {
    # # Calculate MEMs - here we choose the expansion route overlap measure for the distance matrix
    # # A single (the first) eigenvector, i.e. MEM1 of this spatial-weighted matrix explains almost the entirety of the structure/variation here (see eigenvalues below). Further, MEM1 reflects expansion history almost identically to the PCA (PC1 vs PC2) and psi results (show plots).
    # mem_db <- dbmem(as.dist(traj_overlap[[2]]), MEM.autocor = "positive", silent = FALSE) # using default threshold value, i.e. the longest edge of the minimum spanning tree
    # To see what this threshold value is:
    #give.thresh(coords_mat)
    # #saveRDS(mem_db, paste0(working_dir,"trajOverlap_MEM.rds"))
    # Import previous MEM results
    #mem_db <- readRDS(paste0(working_dir,"trajOverlap_MEM.rds")) # this is transC
    mem_db <- readRDS(paste0(working_dir,"trajOverlap_transR_MEM.rds"))
    
    if (plot_figures == "ALL") {
      # Eigenvalues associated to MEMs
      barplot(attr(mem_db, "values"), main = "Eigenvalues of the spatial weighting matrix", cex.main = 0.7)
      # Visualise MEMs in space
      plot(mem_db[,c(1, 2, 3)], SpORcoords = pop.sp)
      #s.value(pop.coords, mem_db[,c(1, 2, 3)], symbol = "circle", ppoint.cex = 0.6)
      # Calculate Moran's I
      moranI <- moran.randtest(mem_db)
      head(attr(mem_db, "values") / moranI$obs)
    }
    # Re-order MEM dataframe to be consistent with allele dataframe (note/caution, this is again different than the pop order of  pop_geodata)
    pop_mem <- cbind(pop_geodata_alt[,c(1,2,16,17)], as.data.frame(mem_db$MEM1))
    pop_mem <- pop_mem[match(allele_data$x, pop_mem$lon),] # lat/lon values are unique
    #pop_mem$lon == allele_data$x # check ordering
    #pop_mem$lat == allele_data$y # check ordering
    colnames(pop_mem)[ncol(pop_mem)] <- "MEM1"
  }
}
...
if (combined_gradientForest == FALSE) {
  ...
  } else if (spatialCovar == "MEM") {
    CLIM_allele <- data.frame(CLIM.extract, pop_mem[,c(3,4,5)], allele_data)
    CLIM_allele <- CLIM_allele[,-c(11,12,13,14,16,17)]# remove lat/lon
    preds <- c(names(CLIM.extract[,-c(11,12)]), "MEM1") # remove lat/lon
  }
...
############## This second section combines parallel (subset) runs of gradientForest into one gradientForest model (object) ##############
...
############## Import combined gradientForest results (that were ran on cluster) ##############
...
if (combined_gradientForest == TRUE) { 
  ...
  if (spatialCovar == "LATLON") {
    ...
    gfVars <- readRDS(paste0("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/gfModel_cor0.5_selectedPredVars_wMEMtransC_combined_method2standardizeBefore_allSplits.rds")) # for MEM, choose transC over transR
    #gfVars <- readRDS(paste0("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/gfModel_cor0.5_selectedPredVars_wMEMtransR_combined_method2standardizeBefore_allSplits.rds"))
  }
}
...
############## This section looks at the raw gradient forest results ############## 
...
if (plot_figures == "ALL") {
  ...
  if (combined_gradientForest == FALSE) {
    ...
    if (spatialCovar == "LATLON") {
      r2_allpreds[,ncol(r2_allpreds)+1] <- rowSums(r2_allpreds[,1:12]) # with lat/lon
    } else if (spatialCovar == "MEM") {
      r2_allpreds[,ncol(r2_allpreds)+1] <- rowSums(r2_allpreds[,1:11]) # with MEM1
    }
    #r2_allpreds[,ncol(r2_allpreds)+1] <- rowSums(r2_allpreds[,1:13])        
...
############## This section project the gradient forest model in space/time ############## 
...
if (spatialCovar == "MEM") {
  # Let's try add interpolated MEM1
  final.ipdw.interp <-raster("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/trajOverlap_MEM_interp.grd")
  #final.ipdw.interp <-raster("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/trajOverlap_MEMtransR_interp.grd")
  #final.ipdw.interp <-raster("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/trajOverlap_MEMtransC_range100_interp.grd")
  names(final.ipdw.interp) <- "MEM1"
  compareRaster(CLIM.alps, final.ipdw.interp)
  CLIM.alps <- stack(CLIM.alps, final.ipdw.interp)
  CLIM.alps.LGM <- stack(CLIM.alps.LGM, final.ipdw.interp)
}
...
ELEV.alps.2 <- mask(ELEV.alps, ELEV_RASTERS$TEMP_MEAN_ANNUAL)
prj_longlat <- "+init=epsg:4326"
crs(ELEV.alps) <- crs(prj_longlat)
crs(ELEV.alps.2) <- crs(prj_longlat)
...
ELEV.alps.2 <- crop(ELEV.alps.2, e_alps)
...
if (spatialCovar == "LATLON") {
  ...
  vars.woLatLon <- vars [! vars %in% c("MEM1")]
}
...
PCs <- prcomp(CLIM.alps.df.GF[, vars.woLatLon], scale. = FALSE, center = TRUE, rank. = 10)
...
if (spatialCovar == "MEM") {
  # Let's mask by SDM hull and plot
  par(mar = c(2, 2, 2, 5), mfrow = c(1, 1))
  final.ipdw <-raster("/Users/luqman/Desktop/Thesis/Phylogeography manuscript/Nature Communications/Revisions/trajOverlap_MEMtransC_range10_interp.grd")
  final.ipdw.cropped <- crop(final.ipdw, extent(CLIM.alps.GF.dist.rast))
  final.ipdw.cropped.masked <- mask(final.ipdw.cropped, current.mask.polygon.wBuffer, updatevalue = NA)
  CLIM.alps.GF.dist.rast.maskedPolygon <- mask(CLIM.alps.GF.dist.rast, final.ipdw.cropped.masked, updatevalue = NA)
  plot(ELEV.alps.2, col = color.palette(100), maxpixels=1e7, legend = FALSE)
  plot(CLIM.alps.GF.dist.rast.maskedPolygon, col=col_pal(n=color_levels), breaks=col_breaks_1, axes=TRUE, legend=F, maxpixels=1e7, add=TRUE)
  plot(current.mask.polygon.wBuffer, add = TRUE, lwd=10)
  plot(CLIM.alps.GF.dist.rast.maskedPolygon, zlim=c(0,0.04), col=col_pal(n=color_levels), legend.only=T)
}
